...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get all users who have entries for last month but no summary yet
    ...
    if (!usersWithEntries || usersWithEntries.length === 0) {
      return NextResponse.json({ message: "No users with entries for last month" });
    }
    ...
    return NextResponse.json({
      message: `Generated ${summariesGenerated} monthly summaries`,
    });
  } catch (error) {
